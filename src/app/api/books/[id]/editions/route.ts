...
async function translateText(text: string, language: string): Promise<string> {
  // Constructed lazily (inside the function) so the module can be imported at
  // build time without OPENAI_API_KEY being present. Matches the pattern used
  // in /api/ai/edit and /api/books/[id]/ai-chat.
